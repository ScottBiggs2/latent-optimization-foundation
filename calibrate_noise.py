...
This script measures the usable band directly: perturb every decoder block at each
...
def perturb_model(model, arch: str, originals, schemas, s: float,
                  weight_std: float, seed: int) -> None:
    """Overwrite every decoder block with w_0 + s * weight_std * eps (in place)."""
    layers = get_layers(model, arch)
    rng = np.random.default_rng(seed)
    for i, layer in enumerate(layers):
        flat = originals[i]
        noise = rng.standard_normal(flat.shape[0]).astype(np.float32)
        reconstruct_block((flat + s * weight_std * noise).astype(np.float32),
                          layer, schemas[i])


def restore_model(model, arch: str, originals, schemas) -> None:
    for i, layer in enumerate(get_layers(model, arch)):
        reconstruct_block(originals[i], layer, schemas[i])


def calibrate_arch(arch: str, scales: List[float], seq_len: int, n_sequences: int,
                   exclude_1d: bool, hf_cache: str, seed: int) -> dict:
    ...
    stack = np.concatenate(originals)
    weight_std = float(stack.std())
    D = int(stack.size)
    del stack

    print(f"  [{arch}] L={len(layers)}  D={D:,}  weight_std={weight_std:.6g}")
    base = compute_perplexity(model, loader, device)
    print(f"  [{arch}] original PPL = {base['perplexity']:.4f}")

    rows = []
    for s in scales:
        perturb_model(model, arch, originals, schemas, s, weight_std, seed)
        ...
        rel_l2 = s * weight_std * np.sqrt(D) / (np.linalg.norm(
            np.concatenate(originals)) + 1e-30)
        ...
        restore_model(model, arch, originals, schemas)
    ...
    return {"arch": arch, "n_layers": len(layers), "n_params": D,
